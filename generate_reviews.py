...
def slugify(text: str, max_length: int = 100) -> str:
    # keep lower-case letters, numbers, dashes
    text = text.lower()
...
def pa_api_search_items(keywords: str, item_count: int = 1) -> dict:
    """
    Sends a signed SearchItems request to PA-API v5.
    ...
    amz_date = t.strftime("%Y%m%dT%H%M%SZ")   # e.g., 20251207T210845Z
    datestamp = t.strftime("%Y%m%d")          # e.g., 20251207
...
def get_trending_products(num: int = 5) -> List[Tuple[str, str]]:
    """
    Use GROQ to propose trending product names + exact Amazon search terms.
    Returns list of tuples [(Product Name, Search Term), ...]
    """
    prompt = f"""
    ...
    for line in raw.splitlines():
        ...
        if not line or "|" not in line:
            continue
        # ignore lines that start with bullets/numbers
...
def generate_review(product_name: str, search_term: str) -> Tuple[str, str, str]:
    """
    Returns (filename, title, image_url)
    """
    ...
    if api_result:
        # PA-API responses can vary; robust extraction:
        try:
            # Standard path: SearchResult -> Items -> list of Item objects
            items = api_result.get("SearchResult", {}).get("Items") or api_result.get("Items") or []
            if isinstance(items, dict):
                # some wrappers use {'Items':{'Item': [...]}} - handle defensively
                items = items.get("Item") or items.get("Items") or []
            if items:
                item = items[0] if isinstance(items, list) else items
                # Title
                title = (item.get("ItemInfo", {}).get("Title", {}).get("DisplayValue")
                         or item.get("ASIN") or product_name)
                # Image
                image_url = (item.get("Images", {}).get("Primary", {}).get("Medium", {}).get("URL")
                             or item.get("Images", {}).get("Primary", {}).get("Large", {}).get("URL")
                             or image_url)
                # Direct link
                direct_link = item.get("DetailPageURL") or direct_link
        except Exception as e:
            print("Warning: unexpected PA-API response structure:", e)
...
def update_homepage(new_reviews: List[Tuple[str, str, str]]):
    ...
    # Insert newest reviews at the top
